...
) : ControllerBase
{
  ...
  public async Task<IActionResult> ChangePassword([FromBody] ChangePasswordDto changePasswordDto)
  {
    try
    {
      ...
      var validation = changePasswordValidator.Validate(changePasswordDto);
      if (!validation.IsValid)
      {
        HttpContext.Items[ErrorFieldConstants.FluentValidationErrors] =
          validation.ToFluentValidationErrorResult();
      ...
      var response = await userService.ChangePasswordAsync(userId, changePasswordDto);

      if (!response.Success)
      {
        return BadRequest(new ApiResponse<bool>(false, response.Message, false));
      }
  ...
  public async Task<IActionResult> UploadProfilePicture(IFormFile file)
  {
    try
    {
      ...
      {
        return BadRequest(new ApiResponse<bool>(false, "No file uploaded", false));
      }

      var userId = User.GetUserId();
      var response = await userService.UploadProfilePictureAsync(userId, file);
    ...
    {
      logger.LogError(ex, "Failed to upload profile picture");
      throw;
    }
  }
}
